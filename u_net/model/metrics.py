...
class CarbonLoss(nn.Module):
    def __init__(self, weight=None, size_average=True, cls_lambda=50, reg_lambda=0.005, ignore_label=255):
        super().__init__()
    ...
    def forward(self, input_cls, input_reg, target_cls, target_reg):
        ...
        cls_loss = self.ce(input_cls, target_cls.type(torch.long).view(input_cls.size(0), input_cls.size(2), input_cls.size(3)))
        ...
        acc_c = corr_sum / idx if idx > 0 else 0.0
        acc_r = r_sum / idx if idx > 0 else 0.0
